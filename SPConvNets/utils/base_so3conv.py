...
from vgtk.functional import compute_rotation_matrix_from_quaternion as rotation_mapping
...
class ClsOutBlockPointnet(nn.Module):
    def __init__(self, params, norm=None, debug=False):
        ...
        if self.pooling_method == 'attention':
            ...
            self.register_buffer("trace_idx_ori", torch.tensor(trace_idx_ori, dtype=torch.long), persistent=False)
            self.register_buffer("trace_idx_rot", torch.tensor(trace_idx_rot, dtype=torch.long), persistent=False)
