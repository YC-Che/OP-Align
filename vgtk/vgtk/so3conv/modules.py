...
class BasicS2Conv(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, anchor_size, debug=False):
        ...
        self.register_buffer('trace_idxv_ori', torch.tensor(trace_idxv_ori.astype(np.int64)), persistent=False)   # 12(na)*13(nk)
        self.register_buffer('trace_idxv_rot', torch.tensor(trace_idxv_rot.astype(np.int64)), persistent=False)

        self.register_buffer('trace_idx_ori', torch.tensor(trace_idx_ori.astype(np.int64)), persistent=False) # 12(na rotations)*12(na channels)
        self.register_buffer('trace_idx_rot', torch.tensor(trace_idx_rot.astype(np.int64)), persistent=False)
        ...
        self.register_buffer('idxs_k', idxs_k, persistent=False)  #   c2, c1, k, a(channels), a(rotations)
        self.register_buffer('idxs_a', idxs_a, persistent=False)  #   c2, c1, k, a(channels), a(rotations)
...
class BasicS2ConvV2(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, anchor_size, debug=False) -> None:
        ...
        self.register_buffer('idx_map', idx_map, persistent=False)
...
class KernelPropagation(nn.Module):
    def __init__(self, dim_in, dim_out, n_center, kernel_size, radius, sigma, kanchor=60):
        ...
        self.register_buffer('kernels', torch.from_numpy(kernels), persistent=False)
...
class S2Conv(nn.Module):
    def __init__(self, dim_in, dim_out, 
        ...
        self.register_buffer('anchors', torch.tensor(anchors).to(torch.float32), persistent=False)
        self.register_buffer('kernels', torch.tensor(kernels).to(torch.float32), persistent=False)
...
class IntraSO3Conv(nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        self.basic_conv = BasicSO3Conv(dim_in, dim_out, self.kernel_size)
        self.register_buffer('anchors', torch.from_numpy(anchors), persistent=False)
        self.register_buffer('intra_idx', torch.from_numpy(intra_idx).long(), persistent=False)
